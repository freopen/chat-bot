...
async fn process_message(context: &UpdateWithCx<AutoSend<Bot>, Message>) -> Result<()> {
  ...
  if let Ok(command) = FreopenBotCommand::parse(
    ...
    if let Some(photo) = photo {
      bot
        .send_chat_action(context.chat_id(), ChatAction::UploadPhoto)
        .await?;
...
async fn message_handler(cx: UpdateWithCx<AutoSend<Bot>, Message>) -> Result<()> {
  if let Err(err) = process_message(&cx).await {
    let _ = cx
      .answer("Something went wrong. Please try again later.".to_string())
      .await
...
pub async fn listen() -> Result<()> {
  ...
  let mut dispatcher = Dispatcher::new(bot).messages_handler(|rx| {
